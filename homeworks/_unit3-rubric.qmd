...
| **C1** `w_mle` == OLS weights on the shipped fixture (rel tol) *(new: merged from Checkpoint 1.2)* | CP 1.2 | autograder | `test_w_mle_equals_ols` | independent |
| **C2** `sigma2_mle` = mean squared residual at `w_mle` *(new: merged from Checkpoint 1.2)* | CP 1.2 | autograder | `test_sigma2_mle_is_mean_sq_residual` | independent |

5 MUST-MEET (A2, B2, B5, D1, D2) + 6 independent (A1, B1, B3, B4, C1, C2); threshold ≤ 1.
